...
def process_pdf():
    ...
    prompt="You are an expert attorney. Give your advice on the following question: "
    located=" I am located here: "
    located=located+location
    prompt=prompt+question+located
    print (prompt)

    if file and file.filename != '':
        filename = secure_filename(file.filename)
        file.save(os.path.join("/tmp", filename)) 
        answer = process_document_and_query(os.path.join("/tmp", filename), question, prompt)
    else:
